...
# 编写任务而不是用户故事（Write Tasks Not User Stories）
...
写 issue 的目的是为了传达工作。它需要足够清晰，以便被指派者能够执行，同时也要提供足够的背景，以便团队成员能够了解正在进行的工作。因此，写 issue 时的目标应该是尽可能有效和快速地完成这个工作。

![Linear Method Write tasks not user stories](images/write-tasks-not-user-stories.png)

## 为什么用户故事已经过时了（Why user stories are obsolete）
...
## 撰写 issue 的更好方法（A better way to write issues）
...
编写清晰、简单的 issue，用平实的语言描述。自己提交自己的 issue。在产品和功能层面讨论用户体验，而不是在实现层面讨论。与其花时间创建用户故事，不如花时间与用户交谈，在构建功能前思考。

### 描述具体的问题（Describe concrete tasks or problems）
...
Issue 应该描述一个有明确结果的工作。这可能是一段代码、设计、文件或要采取的行动。如果它不是一个任务，那么它就不应该被记录到 issue 列表。也许它是一个想法，需要记录在文档或者直接去沟通，或者是一个较大的功能，应该被分解成较小的、有形的任务。
...
这条规则也会有例外。例如，在进行一项功能工作之前，你会花时间探索设计和技术方法。在这种情况下，你可以创建一个占位 issue，以便以后分解（如探索设计），或将其作为一个可交付的事务（如编写项目规格）。

### 清楚和直接（Write clearly and directly）
...
写短而简单的 issue 标题，直接说明任务是什么。标题应该容易扫描，因为大多数人都会在列表上来阅读它。描述应该是可选的，而不是必须的，可以包括相关的想法或背景，以及更深入的讨论的链接。只写执行任务需要知道的内容，其他相关内容可以直接和团队沟通。
...
### 提交自己的 issue（Write your own issues）
...
团队中的每个人都应该自己提交自己的 issue。让更了解问题的人写问题，会更快更容易。这也为你的团队做更好的工作做了准备。当你自己写 issue 时，你被迫在深层次上思考问题。这为提出更好的方案创造了空间，并使你更容易发现计划中的捷径或缺失部分。这种做法也完全重塑了你对待工作的方式。你的重点是产品或项目的可交付性，而不是为了标记任务的完成或核对需求的清单。
...
在某些情况下，为别人写 issue 更有意义，比如在提交错误报告时。我们应该鼓励这种做法，但要注意 issue 的写法略有不同。在写 issue 的时候，把它写成一个请求或者仅仅描述下问题本身。让实际解决的人去想解决方案，然后把 issue 改写成一个任务。

### 将用户体验的讨论保持在产品层面上（Keep user experience discussions at the product level）
...
### ✨ 小提示：我们在 Linear 如何工作（ProTip: How we work at Linear）
...
在决定实现方案之前，我们会深入讨论一个功能。项目负责人撰写规范并收集反馈，直到我们觉得我们有了正确的方法。只有到那时，我们才开始写代码。在建立一个功能之前，花几周的时间来思考是很常见的，但是一旦我们想出了正确的计划，就会直接进入执行模式。项目负责人授权工作，首先从每个人写自己的 issue 开始。
